...
# hand-authored file cannot pose as generated output.
check_artifact_set() {
  local committed expected f
  committed=$(find Zcash/Snark/Fixtures -type f \
    \( -name 'Fixture.lean' -o -name 'proof-bytes.hex' \) | LC_ALL=C sort)
  ...
  while IFS= read -r f; do
    [[ "$f" == *.lean ]] || continue
    if ! head -n 1 "$f" | grep -qE '^-- Auto-generated by halo2 `dump_vesta_lean_fixture(_match_only)?`\. Do not edit by hand\.$'; then
