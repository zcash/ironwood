...
pairs=(
  ...
  # source file at the pinned release, taken by `git show` so it is bound to the tag like the
...
# The pinned key description straight from the release tree, before the capture run.
