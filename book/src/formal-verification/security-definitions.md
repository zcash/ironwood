...
flowchart TD
  ...
  STMT -. "<a target='_blank' href='https://github.com/zcash/ironwood/blob/main/Zcash/Snark/Soundness/Composition/Bridge.lean'>justified by<br/>the extractor;<br/>ledger-bridge<br/>composition open</a>" .-> KS["Knowledge soundness:<br/>accepting proof yields<br/>witness or break data"]
...
<span style="color:#9a6700">■</span> partly machine-checked; remainder tracked (discharging the capstones' named ε's end to end: composing the per-arm oracle-model discharges into one experiment, RedDSA unforgeability; composing knowledge soundness into the ledger model — the extracted witnesses reach the games only through the bridge, where the Sinsemilla ⊥-escapes carry as break data)<br/>
...
proof. The dashed edge marks that justification. The Clean/Ironwood circuit-correctness
conditions (`TopLevelCircuitCorrectness`) are discharged for the deployed Action circuit
(`Circuits/Integration/ActionEncoding.lean`), so extraction ends at `ActionSpec` witnesses
carried as data; what the dashed edge still crosses is the composition of those witnesses into
the ledger model — `Security/Ledger/Bridge.lean` is deliberately the only translation point,
and `ActionSpec`'s `HashGuarded` Sinsemilla conjuncts, vacuous on escaped (⊥) chains, become
break data only there. No single theorem yet composes the SNARK stack with the ledger games.
...
proven reductions. No Lean theorem instantiates the soundness endpoints at the deployed
parameters: the endpoints are stated over the sampled basis, and identifying halo2's
hash-to-curve outputs with that sample is exactly where the heuristic enters
(`Zcash/TrustBoundary.lean` records the scoping). The same heuristic underlies every use of hash-to-curve for fixed bases
...
<div class="g"><div class="g-head"><span class="term">events as branch preimages</span><span class="anchor">Model.balanceSubsetBreakEvent · txBalanceBreakEvent</span></div><div class="def">An adversary is a <code>PMF</code> over valid annotated ledgers; each event is "the computed reduction lands in this branch on this sample", so no choice is needed to extract break data. Violation events are contained in unions of break events, and each break event's probability is a named ε hypothesis. The valid-ledger sample space is inhabited at every instantiation of the primitives (the empty ledger, <code>validLedger_nil</code>); a transaction-bearing valid ledger at the deployed pool — RedPallas wired into <code>Pool.primitives</code>, plus a value-carrying honest spend — is not yet exhibited, so the games' non-degenerate content at a given instantiation rests on the consumer supplying one.</div></div>
...
<div class="g"><div class="g-head"><span class="term">birthday bound</span><span class="anchor">KeyBinding.queries_pair_collision_measure_le · Birthday.birthday_closed_form</span></div><div class="def">The Layer-C probability: the shifted <code>±</code>-collision event over the adversary's own <code>q</code>-bounded query log has probability at most <code>q(q-1)/|𝔽|</code>, by union-bounding the per-pair fraction <code>2/|𝔽|</code>. Counted in the random-oracle model with no hardness assumption; the probability statement over the uniform oracle table is <code>queries_pair_collision_measure_le</code> (#73), with <code>birthday_closed_form</code> the closed-form arithmetic identity behind its union-bound sum.</div></div>
</section>
