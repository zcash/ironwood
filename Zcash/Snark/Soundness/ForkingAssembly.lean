...
2. **generator/value fold recursion** — `foldAll`, `foldGens`, `computeS_cons` already give the `g`/`b`
   one-round fold (`Soundness.DeployedFold`, `Soundness.IpaSoundness`).
...
structure and reconciles to the `DeployedIpaAcceptV` leaf. What stays the (irreducible) residual is step 6's
cryptographic core together with the forking itself — the special-soundness *extraction* (three
distinct-challenge transcripts pin the round point's `(g,U,W)` representation by Vandermonde) and the
random-oracle rewinding that *produces* those transcripts. The structural fold this module proves is the
part downstream of having the representation; the extraction and the rewinding are the honest floor.
-/
