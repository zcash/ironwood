...
The deterministic tree assembly (`Soundness.ForkingAssembly.forkAccept_to_acceptV`) turns the *forking
...
* `extractable_of_prob` — **the multi-round forking lemma.** If, over the random-oracle-uniform challenge
  *vector* `Fin d → α`, the prover's accept probability exceeds the knowledge error
  ...
  `Soundness.TreeExtraction.extractable_of_kerr_lt`: beating the error in *probability* is beating the
