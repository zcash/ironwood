...
`RandomOracle`) — that the challenges are uniform, independent, rewindable random-oracle draws. The
round-by-round transcript ordering (issue #23) is proven and sealed to the deployed derivation
(`Soundness.TranscriptOrdering`, `deriveChallenges_ipaRound_eq`; `proverRoundPoint_proverOfRounds` for the
tree side), so what remains beyond the uniformity axiom is the adversary-execution semantics — that a rewound
adversary *is* such a prefix-respecting `Prover` — not the ordering. -/
