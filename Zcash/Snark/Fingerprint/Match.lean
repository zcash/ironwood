...
* byte→algebraic decode — `DeployedAcceptsBytes` composes exact `readProof?` parsing, the derived
  digest, BLAKE2b challenge derivation, and typed `DeployedAccepts`; `readProof?` is canonical and
  reads all four captures' generated bytes to their typed proofs. The deployed reader is exercised
  on those exact captures and edits, not universally refined to the Lean function;
