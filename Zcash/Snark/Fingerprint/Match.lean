...
* byte→algebraic decode — `DeployedAcceptsBytes` composes exact `readProof?` parsing, the derived
  digest, BLAKE2b challenge derivation, and typed `DeployedAccepts`, under the key identification
  `Describes` (the pinned description's constraint system is the key's and the shape's) and
  halo2's refusal of identity instance commitments; `readProof?` is canonical and reads all four
  captures' generated bytes to their typed proofs. The deployed reader is exercised on those exact
  captures and edits, not universally refined to the Lean function, and `blindingFactors`,
  `delta`, and `chunkLen` stay the named key agreements of `Verifier/Key.lean`;
...
(`Verifier/KeyDigest.lean`) to the captured `transcript_repr`, each honest family discharges
`Describes` — the description's fields are the captured key's and shape's — as the conjunct
`DeployedAcceptsBytes` requires, and `Fixtures/PinnedKey.lean` states that reading field by field.
The string, captured key, and Orchard's own key-description comparison stay at the
